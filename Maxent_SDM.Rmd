...
**`r recs` records** of `r params$Species` are used for processing (georeference obscurity < 500m).

```{r Records plot, include=TRUE, echo=FALSE, dev='png'}

all_countries <- rnaturalearth::ne_countries(scale = "medium", returnclass = "sf")
gg <- ggplot() + geom_point(data = points, aes(x=decimalLongitude, y = decimalLatitude), col="blue", size = 0.7) +
...
env_datasets <- list(
  predictors = predictors_cleaned,
  worldclim = worldclim_data_aligned_cleaned,
  landuse = landuse_rasterbrick_cleaned
)
...
for (env_name in names(env_datasets)) {
  ...
  best_models[[env_name]] <- eval_results[[env_name]]@models[[bestmod]]
...
model_bioclim <- maxent(
  worldclim_data_aligned_cleaned,
  pres_train,
  removeDuplicates = TRUE,
  replicates = 10,
...
```{r Plotting predictor contributions, include=TRUE, echo=FALSE, dev='png'}
...
```{r Plotting predictor response curve: full model, include=TRUE, echo=FALSE, dev='png'}
...
```{r Plotting predictor response curve: BioClim model, include=TRUE, echo=FALSE, dev='png'}
plot_response_curves(model_bioclim)
```

#### Land Use model

```{r Plotting predictor response curve: Land Use model, include=TRUE, echo=FALSE, dev='png'}
...
```{r Plotting model accuracy, include=TRUE, echo=FALSE, dev='png'}
...
if (params$Range == "YES") {
range_raster <- raster::raster(range)
range_raster_resampled <- resample(range_raster, present_full, method = "ngb")

present_full <- raster::mask(present_full, range_raster_resampled)
present_bioclim <- raster::mask(present_bioclim, range_raster_resampled)
present_landuse <- raster::mask(present_landuse, range_raster_resampled)

fut_full_pred <- raster::mask(fut_full_pred, range_raster_resampled)
fut2_full_pred <- raster::mask(fut2_full_pred, range_raster_resampled)
fut_bioclim_pred <- raster::mask(fut_bioclim_pred, range_raster_resampled)
fut2_bioclim_pred <- raster::mask(fut2_bioclim_pred, range_raster_resampled)
}

```


```{r plotting the present 1/3, include=TRUE, echo=FALSE, dev='png'}
...
plot(spdf, add = TRUE, pch = 16, cex = 0.5, col = rgb(red = 0, green = 0, blue = 1, alpha = 0.3))
```

### SDM using BioClim data only


```{r plotting the present 2/3, include=TRUE, echo=FALSE, dev='png'}
# plot(hill, col=grey(0:100/100), legend=FALSE, mar=c(2,2,1,4),
#      xlim = c(ext.pred@xmin, ext.pred@xmax),
#      ylim = c(ext.pred@ymin, ext.pred@ymax))
...
```{r plotting the present 3/3, include=TRUE, echo=FALSE, dev='png'}
# plot(hill, col=grey(0:100/100), legend=FALSE, mar=c(2,2,1,4),
#      xlim = c(ext.pred@xmin, ext.pred@xmax),
#      ylim = c(ext.pred@ymin, ext.pred@ymax))
...
```{r plotting the future 1/2, include=TRUE, echo=FALSE, dev='png'}
# plot(hill, col=grey(0:100/100), legend=FALSE, mar=c(2,2,1,4),
#      xlim = c(ext.pred@xmin, ext.pred@xmax),
#      ylim = c(ext.pred@ymin, ext.pred@ymax))
...
```{r plotting the future 2/2, include=TRUE, echo=FALSE, dev='png'}
# plot(hill, col=grey(0:100/100), legend=FALSE, mar=c(2,2,1,4),
#      xlim = c(ext.pred@xmin, ext.pred@xmax),
#      ylim = c(ext.pred@ymin, ext.pred@ymax))
...
```{r plotting the change, include=TRUE, echo=FALSE, dev='png'}
# CHANGE
...
```{r plotting the limiting factors, include=TRUE, echo=FALSE, dev='png'}
